...
namespace lectia3simult
{
    ...
    {   
        static int [,]load(string fileName)
        {
            ...
            dataLoad.Close();
            lines = data.Count;
            colums = (data[0].Split(' ')).Length;
            int[,] toReturn = new int[lines, colums];
            ...
            while (!A.isEmpty())
            {
                ...
                if (local.l - 1 >= 0)
                    if (a[local.l - 1, local.c] == 0)
                    {
                        ...
                        A.Push(new Data(local.l - 1, local.c, local.v + 1));
                    }
                if (local.c + 1 < a.GetLength(1))
                    if (a[local.l, local.c + 1] == 0)
                    {
                        a[local.l, local.c + 1] = local.v + 1;
                        A.Push(new Data(local.l, local.c + 1, local.v + 1));
                    }
